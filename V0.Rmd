...
```{r}
#retval <- subset(datos_us, "duration (seconds)" > 1200, select = c(state, `duration (seconds)`))
datos_us <- datos_us[!is.na(datos_us$"duration (seconds)"), ]#eliminar NAs
intervalo <- t.test(datos_us$"duration (seconds)", conf.level = 0.95)
print(intervalo$conf.int)
print(paste("El resultado indica que la media de la variable duracion (seconds) es de ", mean(datos_us$"duration (seconds)"), " el cual se encuentra con una confianza del 95% en el intervalo ", intervalo$conf.int[1], " ", intervalo$conf.int[2]))
```

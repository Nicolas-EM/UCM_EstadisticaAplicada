...
```{r}
by_city <- aggregate(datos_us$city, by=list(Country = datos_us$city), FUN=length)
mostseek<-by_city[which(by_city$x>4),]

maxcity<-by_city[which(by_city$x == max(by_city$x)),]
print(paste("La ciudad con mayor probabilidad de avistar un ovni es",maxcity$Country,"con una probabilidad de un ",(maxcity$x/count(by_city))*100,"%"))


ggplot(mostseek, aes(x="", y=mostseek$x, fill=mostseek$Country)) +
  geom_bar(stat="identity", width=1) +
  coord_polar("y", start=0) +
  xlab("") +
  ylab("") +

...
### Probabilidad por pais
Probabilidad de ser de cada pais en caso de avistar un ovni.

```{r}
numAvistamientos <- sum(by_country$x)

probAvis <- (by_country[which(by_country$Country == 'au'),]$x/numAvistamientos)*100
print(paste("La probabilidad de avistar un ovni en Australia es del: ",probAvis,"%"))

probAvis <- (by_country[which(by_country$Country == 'ca'),]$x/numAvistamientos)*100
print(paste("La probabilidad de avistar un ovni en Canadá es del: ",probAvis,"%"))

probAvis <- (by_country[which(by_country$Country == 'de'),]$x/numAvistamientos)*100
print(paste("La probabilidad de avistar un ovni en Alemania es del: ",probAvis,"%"))

probAvis <- (by_country[which(by_country$Country == 'gb'),]$x/numAvistamientos)*100
print(paste("La probabilidad de avistar un ovni en Gran Bretaña es del: ",probAvis,"%"))

probAvis <- (by_country[which(by_country$Country == 'us'),]$x/numAvistamientos)*100
print(paste("La probabilidad de avistar un ovni en Gran Bretaña es del: ",probAvis,"%"))

probAvis <- (by_country[which(by_country$Country == 'otros'),]$x/numAvistamientos)*100
print(paste("La probabilidad de avistar un ovni en Gran Bretaña es del: ",probAvis,"%"))
```
...
by_state <- aggregate(datos_us$city, by=list(State = datos_us$state), FUN=length)
by_state <- by_state[with(by_state,order(-x)),]
avistTot <- by_country[which(by_country$Country == 'us'),]$x

print(paste("El estado con mayor probabilidad de avistar un ovni es",(by_state[1,])$State,"con una probabilidad de un ",((by_state[1,])$x/avistTot)*100,"%"))

```

A continuación se muestra una gráfica con los estados donde mayor probabilidad hay de avistar un ovni.

```{r}
ggplot(by_state[1:10,], aes(x=(by_state[1:10,])$State, y=((by_state[1:10,])$x/avistTot)*100, fill=(by_state[1:10,])$State)) +
  geom_bar(stat="identity", width=1) +
  xlab('') +
  ylab('Probabilidad') +

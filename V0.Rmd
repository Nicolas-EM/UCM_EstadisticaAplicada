...
probinter1 <- datos_us[which(datos_us$city == 'seattle'&format(datos_us$datetime,"%H")==22),]
probinter2<-aggregate(probinter1$city,by=list(city=probinter1$city),FUN=length)
probinter3 <- datos_us[which(datos_us$city == 'seattle'|format(datos_us$datetime,"%H")==21),]
probinter4<-aggregate(probinter3$city,by=list(city=probinter3$city),FUN=length)
probinter5 <- datos_us[which(datos_us$city != 'seattle'&format(datos_us$datetime,"%H")!=21),]
probinter6<-aggregate(probinter5$city,by=list(city=probinter5$city),FUN=length)
probinter5 <- datos_us[which(datos_us$city != 'seattle'&format(datos_us$datetime,"%H")!=21),]
probinter6<-aggregate(probinter5$city,by=list(city=probinter5$city),FUN=length)
probinter7 <- datos_us[which(datos_us$city != 'seattle'&format(datos_us$datetime,"%H")==21),]
probinter8<-aggregate(probinter7$city,by=list(city=probinter7$city),FUN=length)
probinter9 <- datos_us[which(datos_us$city != 'seattle'&format(datos_us$datetime,"%H")==21&(datos_us$shape == 'light'|datos_us$shape=='unknown')),]
probinter10<-aggregate(probinter9$city,by=list(city=probinter9$city),FUN=length)
probinter11 <- datos_us[which(datos_us$city == 'seattle'&format(datos_us$datetime,"%H")!=21&(datos_us$shape != 'light'&datos_us$shape!='unknown')&datos_us$`duration (seconds)`>10),]
probinter12<-aggregate(probinter11$city,by=list(city=probinter11$city),FUN=length)

print(paste("La probabilidad de ver un UFO en seattle a las 22:00 es del",(sum(probinter2$x)/avistTot)*100,"%"))
print(paste("La probabilidad de ver un UFO en seattle o verlo a las 21:00 es del",(sum(probinter4$x)/avistTot)*100,"%"))
print(paste("La probabilidad de ver un UFO en una ciudad que no sea seattle a una hora distinta a las 21:00 es del",(sum(probinter6$x)/avistTot)*100,"%"))
print(paste("La probabilidad de ver un UFO en una ciudad que no sea seattle a las 21:00 es del",(sum(probinter8$x)/avistTot)*100,"%"))
print(paste("La probabilidad de ver un UFO en una ciudad distinta de seattle a las 21:00 con una forma que sea lumínica o desconocida",(sum(probinter10$x)/avistTot)*100,"%"))
print(paste("La probabilidad de ver un UFO en seattle a una hora distinta de las 21:00 con una forma que sea distinta de lumínica o desconocida pero con una duracion de mas de 10 segundos",(sum(probinter12$x)/avistTot)*100,"%"))
...
prob2<-count(prob1)
prob3 <- datos_us[which(datos_us$city == 'richmond'),]
prob4<-count(prob3)
prob5 <- datos_us[which(datos_us$shape=='changing'),]
prob6<-count(prob5)
prob7 <- datos[which(datos$country != 'us'),]
prob8<-count(prob7)


print(paste("Dado que son las 15:00 que probabilidad hay de ver un UFO?",(prob2/avistTot)*100,"%"))
print(paste("Si vivo en Richmond que probabilidad tengo de ver un UFO?",(prob4/avistTot)*100,"%"))
print(paste("En caso de ver un ovni que probabilidad hay de que sea cambiante?",(prob6/avistTot)*100,"%"))
print(paste("Si vivo fuera de Estados Unidos que probabilidad hay de avistar un ovni?",(prob8/maxdat)*100,"%"))
```
